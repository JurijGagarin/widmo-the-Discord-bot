...
//Canvas.registerFont('./photos/LilitaOne-Regular.ttf', { family: 'Lilita One' });
...
var staliNicki = ['Primoz', 'worldshowoff', 'goha', 'hubibibi', 'Vlad', 'rawairek', 'Olos', 'matius', 'araxe', 'Mareque', 'de_presja', 'Kubys', 'gamex', 'Tajkos', 'Doguss', 'Olaga', 'Misiek', 'Janszy', 'KPç', 'Bart', 'Mar', 'Skała', 'michcio', 'naruku', 'Pawlo', 'Bucu', 'Koziełło', 'Chunky', 'seed', 'Rysia', 'rayan', 'zapiekanka', 'Caro', 'Sinnohne', 'Konrad', 'mumin']
...
exports.run = (konkursy, użytkownicy, channel, typ, rokS) => {
  ...
  if(['ind', 'druż', 'duety', 'mn', 'gp', 'bf'].includes(typ)){
  ...
  Promise.all(obietnice).then(async (fotki) => {
    ...
    for(let i = 0; i < konkursy.length; i = i + 10){
      ...
      if(i == 0){
        ...
        for(let j = 0; j < 3; j++) if(j > 0 && konkursy[j].punkty != konkursy[j-1].punkty) nr = j + 1
      }
