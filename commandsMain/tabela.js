  ...
  if(rodzaj == 'q'){
    ...
    if(parametr == 'pingi'){
      ...
      Promise.all(obietniceZawodników).then(nicki => {
        ...
        if(parametr == 's'){
          skokowa.run(konkursy, nicki, message.channel, rodzaj)
        }
        if(parametr == 'żw'){
      ...
      for(let n = 0; n < konkursy.length; n++){
        ...
        for(let m in listaKrajów){
          if(listaKrajów[m] == konkursy[n].kraj){
            if(!listaKodów2[m].startsWith('GB')) konkursy[n].flaga = `flag_${listaKodów2[m].toLowerCase()}`
            ...
            else if(listaKodów2[m] == 'GB-NIR')  konkursy[n].flaga = `northern_ireland`
            znaleziono = true
            break
          }
        }
    ...
    if((parametr == 'pingi'|| parametr == 'nicki') && rodzaj == 'n'){
      ...
      skokowa.run(konkursy, konkursy, message.channel, rodzaj)
    }
    else if(parametr == 'żw'){
